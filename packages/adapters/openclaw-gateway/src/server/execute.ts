...
function buildWakeText(payload: WakePayload, paperclipEnv: Record<string, string>): string {
  ...
  const lines = [
    ...
    "- Prefer `paperclipai issue ...` for checkout/release/comment/update when the CLI is available; it serializes JSON safely and forwards the run id automatically.",
    ...
    "3) If issueId exists:",
    "   - Preferred checkout command: paperclipai issue checkout {issueId} --agent-id \"$PAPERCLIP_AGENT_ID\"",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$PAPERCLIP_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    "   - GET /api/issues/{issueId}",
    "   - GET /api/issues/{issueId}/comments",
    "   - Execute the issue instructions exactly.",
    "   - Preferred comment command: paperclipai issue comment {issueId} --body \"...\"",
    "   - If instructions require a comment, POST /api/issues/{issueId}/comments with {\"body\":\"...\"}.",
    "   - Preferred status update command: paperclipai issue update {issueId} --status done --comment \"what changed and why\"",
    "   - PATCH /api/issues/{issueId} with {\"status\":\"done\",\"comment\":\"what changed and why\"}.",
    "   - Preferred release command: paperclipai issue release {issueId}",
    "   - If you claimed the wrong task, release it with POST /api/issues/{issueId}/release.",
