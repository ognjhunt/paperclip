...
async function writeFakeGeminiCommand(commandPath: string, options: { extraStdoutLines?: string[] } = {}): Promise<void> {
...
describe("gemini execute", () => {
  ...
  it("redacts printenv-style secret output in adapter log chunks and result payloads", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-gemini-redaction-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
    await writeFakeGeminiCommand(commandPath, {
