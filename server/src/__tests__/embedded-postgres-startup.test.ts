...
describe("embedded postgres startup helpers", () => {
  it("classifies an empty postmaster.pid as empty", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-postmaster-empty-"));
    try {
  ...
  it("reuses an instance that becomes reachable while the pid file is settling", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-postmaster-reuse-"));
    try {
      const filePath = path.join(tempDir, "postmaster.pid");
      await writeFile(filePath, "\n", "utf8");
  ...
  it("blocks cleanup while a postmaster pid is still running but unreachable", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-postmaster-running-"));
    try {
      const filePath = path.join(tempDir, "postmaster.pid");
  ...
  it("cleans up stale pid files after settle retries are exhausted", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "paperclip-postmaster-stale-"));
    try {
      const filePath = path.join(tempDir, "postmaster.pid");
